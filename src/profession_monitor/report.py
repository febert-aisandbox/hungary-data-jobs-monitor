...
def build_snapshot(store, run, site_url=SITE_URL, failed_searches=None, expected_queries=None):
...
def render_html(s):
    ...
    coverage = "" if s["status"] == "success" else f'<p><strong>Partial coverage:</strong> {s["completed_queries"]}/{s["expected_queries"]} searches completed. Unobserved jobs were carried forward and no expirations were recorded.</p>'
...
</style></head><body><main><p class="meta">PROFESSION.HU · DAILY MARKET MONITOR</p><h1>Hungary data jobs</h1><p class="meta">Last update: {html.escape(s["updated_at"])}</p>{coverage}
...
def render_digest(s):
    ...
    if s["status"] == "degraded": lines.insert(1,f"⚠️ Partial coverage: **{s['completed_queries']}/{s['expected_queries']} searches** completed; no expirations were recorded.")
    if s["new_jobs"]:
